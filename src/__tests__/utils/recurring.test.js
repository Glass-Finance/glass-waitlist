...
describe("scheduleCopy", () => {
  it("describes a simple monthly plan with a billing day", () => {
    expect(scheduleCopy({ frequency: "MONTHLY", interval: 1, billingDay: 5 })).toBe(
      "Every month on the 5th",
    );
  });

  it("pluralizes the unit when interval > 1", () => {
    expect(scheduleCopy({ frequency: "DAILY", interval: 3 })).toBe("Every 3 days");
  });

  it("names the weekday for a weekly plan with a billingDay 1-7", () => {
    expect(scheduleCopy({ frequency: "WEEKLY", interval: 1, billingDay: 1 })).toBe(
      "Every week on Mondays",
    );
    expect(scheduleCopy({ frequency: "WEEKLY", interval: 2, billingDay: 7 })).toBe(
      "Every 2 weeks on Sundays",
    );
  });

  it("omits the day suffix for DAILY plans even if billingDay is set", () => {
    expect(scheduleCopy({ frequency: "DAILY", interval: 1, billingDay: 5 })).toBe("Every day");
  });

  it("falls back to 'cycle' for an unrecognized frequency", () => {
    expect(scheduleCopy({ frequency: "FORTNIGHTLY", interval: 1 })).toBe("Every cycle");
  });
...
describe("estimateNextCharge", () => {
  ...
  it("adds 3 months per interval for a QUARTERLY plan", () => {
    const next = estimateNextCharge(
      { frequency: "QUARTERLY", interval: 1 },
      "2026-07-01T00:00:00.000Z",
    );
