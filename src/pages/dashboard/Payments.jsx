        ...
        return (
          <div key={s.n} className={`flex items-center ${i < steps.length - 1 ? "flex-1" : ""}`}>
            <div className="flex flex-col items-center gap-1 flex-shrink-0">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center font-medium text-xs border
              ...
              <span className={`text-[11px] whitespace-nowrap ${active ? "font-semibold text-[#1C2B8A]" : done ? "text-gray-600" : "text-gray-400"}`}>
                {s.label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 mb-4 ${done ? "bg-[#1C2B8A]" : "bg-gray-200"}`} />
            )}
          </div>
        );
      })}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { id: "one_time",  icon: <Zap size={22} />,       title: "One Time",  desc: "A single payment for one purpose." },
        ].map((opt) => {
          ...
          return (
            <button key={opt.id} onClick={() => onChange(opt.id)}
              ...
              <div className={`absolute top-3 left-3 w-5 h-5 rounded-full border-2 flex items-center justify-center ${sel ? "bg-[#1C2B8A] border-[#1C2B8A]" : "border-gray-300"}`}>
    ...
    <div className="pr-1 space-y-3.5">
      <div>
        ...
        <input className={inputCls} value={form.name || ""} placeholder="Enter plan name"
          onChange={(e) => onChange("name", e.target.value)} onBlur={() => !slug && suggestFrom(form.name)} />
      </div>
      <div>
      ...
      <div>
        ...
        <textarea className={`${inputCls} resize-y`} value={form.description || ""} rows={3}
      ...
      <label className="flex items-center gap-2 text-xs text-gray-700">
        <input type="checkbox" checked={form.activateImmediately ?? true}
  ...
  return (
    <div>
      <div className="rounded-xl border border-gray-200 overflow-hidden mb-3">
        {rows.map((r, i) => (
          <div key={r.label} className={`flex justify-between px-4 py-2.5 text-xs ${i < rows.length - 1 ? "border-b border-gray-100" : ""} ${i % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-[rgba(15,29,110,0.2)] backdrop-blur-sm"
      ...
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-start justify-between px-6 pt-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 cursor-pointer bg-transparent border-solid">
            <X size={14} />
          </button>
        </div>
        <div className="px-6 py-4 flex-1 overflow-hidden flex flex-col">
          {success ? (
            <div className="text-center py-10">
              ...
              <h3 className="text-lg font-semibold text-black mb-2">Plan Created!</h3>
              <p className="text-sm text-gray-500 mb-6">Members have been notified.</p>
              <button onClick={onClose} className="px-6 py-2 rounded bg-[#1C2B8A] text-white font-medium text-xs cursor-pointer border-none">Done</button>
            </div>
          ) : (
            <>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-[rgba(15,29,110,0.2)] backdrop-blur-sm"
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6">
        <div className="flex items-start justify-between mb-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 cursor-pointer bg-transparent">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div>
            ...
            <input className={inputCls} value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Plan name" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">Amount Per Member</label>
...
function PlanMembersModal({ plan, communityId, onClose }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await getPaymentLinkMembers(communityId, plan.id, { pageSize: 200 });
  ...
  function getPaid(m) { return m.paidCount ?? (m.obligationStatus === "PAID" ? 1 : 0); }
  function getTotal(m) { return m.totalCount ?? 1; }
  ...
  const filtered = members.filter((m) => {
    ...
    if (statusFilter === "Paid" && !(getPaid(m) === getTotal(m) && getTotal(m) > 0)) return false;
    if (statusFilter === "Unpaid" && getPaid(m) !== 0) return false;
    if (statusFilter === "Partial" && !(getPaid(m) > 0 && getPaid(m) < getTotal(m))) return false;
    return true;
  });

  const activeChips = statusFilter ? [{ key: "status", label: statusFilter }] : [];

  function exportCsv() {
    const rows = selected.length ? filtered.filter((m) => selected.includes(m.id ?? getName(m))) : filtered;
    const header = "Name,Email,Paid,Total,Date Joined\n";
    const body = rows.map((m) => `${getName(m)},${getEmail(m)},${getPaid(m)},${getTotal(m)},${formatDate(getJoinedAt(m))}`).join("\n");
    const blob = new Blob([header + body], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `members-${plan.name}.csv`; a.click();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-[rgba(15,29,110,0.2)] backdrop-blur-sm"
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
          <div>
            <h2 className="text-base font-semibold text-black">Members List ({plan.name})</h2>
            <p className="text-xs text-gray-400 mt-0.5">You can edit or pause any plan at any time.</p>
          </div>
          ...
          <div className="flex items-center gap-2 px-6 pb-2">
            {activeChips.map((chip) => (
              <span key={chip.key} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gray-100 text-xs text-gray-700">
                {chip.label}
                <button onClick={() => setStatusFilter("")} className="bg-transparent border-none cursor-pointer p-0 flex items-center">
                  <X size={10} className="text-gray-400" />
                </button>
              </span>
            ))}
            <button onClick={() => setStatusFilter("")} className="text-xs font-semibold text-[#1C2B8A] bg-transparent border-none cursor-pointer">
              Clear All
            </button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-sm border-collapse">
              ...
              <tr className="border-y border-gray-100 bg-gray-50">
                <th className="px-5 py-2.5 w-8">
                  <input type="checkbox"
                    ...
                    onChange={(e) => setSelected(e.target.checked ? filtered.map((m) => m.id ?? getName(m)) : [])} />
                </th>
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={8} className="px-5 py-10 text-center text-xs text-gray-400">Loading…</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="px-5 py-10 text-center text-xs text-gray-400">No members found.</td></tr>
              ) : (
                filtered.map((m, i) => {
                  const key = m.id ?? i;
                  const paid = getPaid(m), total = getTotal(m);
                    ...
                    <tr key={key} className="border-b border-gray-50 hover:bg-blue-50/20 transition-colors">
                      <td className="px-5 py-3">
                        <input type="checkbox" checked={selected.includes(key)}
                          onChange={() => setSelected((p) => p.includes(key) ? p.filter((x) => x !== key) : [...p, key])} />
                      </td>
                      <td className="px-4 py-3 text-xs font-semibold text-[#1C2B8A]">{getName(m)}</td>
                      <td className="px-4 py-3 text-xs text-gray-600">{m.planCount ?? total}</td>
                      <td className="px-4 py-3">
                        <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ color: s.color, background: s.bg }}>
                          {paid}/{total} Paid
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs text-gray-500">{formatDate(m.lastPaymentDate)}</td>
                      <td className="px-4 py-3 text-xs text-gray-500">{getEmail(m)}</td>
                      <td className="px-4 py-3 text-xs text-gray-500">{formatDate(getJoinedAt(m))}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1.5">
                          <button disabled title="Send reminder — coming soon"
    ...
    <div className="px-6 py-6 overflow-y-auto h-full">
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">A full picture of all payments created in your community.</p>
        </div>
