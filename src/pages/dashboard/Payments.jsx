...
function StatCard({ icon: Icon, label, value, color, bg }) {
  return (
    <div
      className="bg-surface-container rounded-xl border border-gray-100 px-4 py-3 flex items-center justify-between"
...
// over from the original automatically, and it validates them the same way
// Create does (confirmed by "Start date cannot be in the past" once startAt
// was added). Mirrors CreatePlanModal's fields for the two things a
// duplicate genuinely needs fresh: a non-colliding title+slug, and dates
// that aren't already in the past. amount/audience/community account/
// reminders aren't in this schema at all, so the backend must carry those
// over from the original itself -- no need to collect them again here.
// For a recurring original, the recurrence shape (frequency/interval/
// billingDay/retryPolicy/graceDays) is carried over unchanged from the
...
function PlanCard({
  ...
  return (
    <div
      className="bg-surface-container rounded-2xl border border-gray-100 p-5 flex flex-col gap-4"
