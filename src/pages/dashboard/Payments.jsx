    ...
    step === 1
      ...
      : step === 2
        ? !!(
            ...
            form.amount &&
            slugState.slug &&
            (planType === "one_time" || form.frequency)
          )
        : true;
      ...
      ...(planType === "recurring"
        ? {
            recurringPlan: {
              ...
              billingDay: new Date(form.startDate || new Date()).getDate(),
            },
          }
        : {}),
      ...(form.dueDate ? { dueAt: new Date(form.dueDate).toISOString() } : {}),
      ...(form.startDate && planType !== "recurring"
...
function PlanActionsMenu({ plan, planPlans }) {
  ...
  if (status === "DRAFT")
    actions.push({
      label: "Activate",
      run: () => planPlans.activate.mutate(plan.id),
    });
  if (status === "ACTIVE")
    actions.push({
      label: "Pause",
      run: () => planPlans.pause.mutate(plan.id),
    });
  if (status === "PAUSED")
    actions.push({
      label: "Resume",
      run: () => planPlans.resume.mutate(plan.id),
    });
  if (status === "ACTIVE" || status === "PAUSED")
    actions.push({
      label: "Expire",
      run: () => planPlans.expire.mutate(plan.id),
    });
  if (status !== "ARCHIVED")
    actions.push({
      label: "Archive",
      run: () => planPlans.archive.mutate(plan.id),
    });
  actions.push({
    label: "Duplicate",
    run: () => planPlans.duplicate.mutate(plan.id),
  });
    ...
    <div className="px-6 py-6 overflow-y-auto h-full">
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        ...
        <StatCard
          ...
          value={String(stats.failed).padStart(2, "0")}
          color="#7c3aed"
          bg="#F3EEFF"
        />
      </div>
